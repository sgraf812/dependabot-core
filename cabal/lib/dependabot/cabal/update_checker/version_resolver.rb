  ...
  module Cabal
    class UpdateChecker
      class VersionResolver
        ...
        def fetch_latest_resolvable_version
          ...
          SharedHelpers.in_a_temporary_directory(base_directory) do
            ...
            SharedHelpers.with_git_configured(credentials: credentials) do
              # Shell out to Cabal, which handles everything for us, and does
              # so without doing an install (so it's fast).
              run_cabal_command("cabal update -p #{dependency_spec} --verbose")
            end
        ...
        def run_cabal_command(command)
          start = Time.now
          command = SharedHelpers.escape_command(command)
          stdout, process = Open3.capture2e(command)
          time_taken = Time.now - start

          # Raise an error with the output from the shell session if Cabal
        ...
        def check_rust_workspace_root
          cabal_project = original_dependency_files.
                       select { |f| f.name.end_with?("../cabal.project") }.
                       max_by { |f| f.name.length }
          return unless TomlRB.parse(cabal_project.content)["workspace"]
        ...
        def handle_cabal_errors(error)
          ...
          if error.message.include?("all possible versions conflict")
            ...
            # updating. It's (probably) a Cabal bug.
            return nil
          end
        ...
        # called a virtual manifest: https://doc.rust-lang.org/cabal/reference/
