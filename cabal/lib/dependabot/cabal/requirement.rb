...
module Dependabot
  module Cabal
    class Requirement < Gem::Requirement
      ...
      version_pattern = Cabal::Version::VERSION_PATTERN

      PATTERN_RAW = "\\s*(#{quoted})?\\s*(#{version_pattern})\\s*"
      PATTERN = /\A#{PATTERN_RAW}\z/.freeze

      # Use Cabal::Version rather than Gem::Version to ensure that
      # pre-release versions aren't transformed.
      def self.parse(obj)
        return ["=", Cabal::Version.new(obj.to_s)] if obj.is_a?(Gem::Version)
        ...
        [matches[1] || "=", Cabal::Version.new(matches[2])]
      end
...
Dependabot::Utils.
  register_requirement_class("cabal", Dependabot::Cabal::Requirement)
