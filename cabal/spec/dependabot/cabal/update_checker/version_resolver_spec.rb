...
require "dependabot/cabal/update_checker/file_preparer"
require "dependabot/cabal/update_checker/version_resolver"

RSpec.describe Dependabot::Cabal::UpdateChecker::VersionResolver do
  subject(:resolver) do
    described_class.new(
  ...
  let(:dependency_files) do
    Dependabot::Cabal::UpdateChecker::FilePreparer.new(
  ...
  describe "latest_resolvable_version" do
    ...
    context "with a dependency that doesn't exist" do
      ...
      context "with some TOML that Cabal can't parse" do
    ...
    context "when there's a virtual workspace" do
      ...
      let(:workspace_child) do
        Dependabot::DependencyFile.new(
          name: "src/sub_crate/cabal.project",
      ...
      let(:requirements) do
        [{
          requirement: "=0.4.0",
          file: "src/sub_crate/cabal.project",
          groups: ["dependencies"],
          source: nil
        }]
      end
    ...
    context "when there is a workspace" do
      ...
      let(:requirements) do
        [{
          requirement: "=0.4.0",
          file: "lib/sub_crate/cabal.project",
          groups: ["dependencies"],
          source: nil
        }]
      end
