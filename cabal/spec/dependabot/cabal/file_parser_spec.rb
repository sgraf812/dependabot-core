    ...
    context "with only a manifest" do
      ...
      context "with a path dependency" do
        ...
        let(:path_dependency_manifest) do
          Dependabot::DependencyFile.new(
            name: "src/s3/cabal.project",
            content: fixture("manifests", "cabal-registry-s3"),
            support_file: true
          )
        end
        ...
        describe "top level dependencies" do
          ...
          describe "the first dependency" do
            ...
            it "has the right details" do
              expect(dependency).to be_a(Dependabot::Dependency)
              expect(dependency.name).to eq("cabal-registry-s3")
      ...
      context "with workspaces" do
        ...
        context "with a virtual workspace root" do
          ...
          let(:workspace_child2) do
            Dependabot::DependencyFile.new(
              name: "src/sub_crate2/cabal.project",
              content: workspace_child2_body
            )
          end
          let(:workspace_child3) do
            Dependabot::DependencyFile.new(
              name: "src/sub_crate3/cabal.project",
              content: workspace_child2_body
            )
          end
              ...
              it "has the right details" do
                ...
                expect(dependency.requirements).to eq(
                  [
                    {
                      requirement: nil,
                      file: "src/sub_crate2/cabal.project",
                      groups: ["dependencies"],
                      source: { type: "path" }
                    },
                    {
                      requirement: nil,
                      file: "src/sub_crate3/cabal.project",
                      groups: ["dependencies"],
                      source: { type: "path" }
                    }
                  ]
        ...
        it "raises a DependencyFileNotParseable error" do
          expect { parser.parse }.
            to raise_error(Dependabot::DependencyFileNotParseable) do |error|
              expect(error.file_name).to eq("cabal.project")
            end
        end
      end
